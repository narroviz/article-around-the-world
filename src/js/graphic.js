...
async function drawGlobalPath(dimesnions, bounds) {
	...
	drawGlobalPathByLatLon(35, -118.4912, DIMENSIONS, BOUNDS)
...
async function drawGlobalPathByLatLon(latitude, longitude, dimensions, bounds) {
	...
	const pathData = await d3.json('./../assets/data/santamonica.json')
	...
	for(var i=0;i<distances.length;i++){
		...
		for (var j = 0; j < Object.keys(countryToIndex).length; j++) {
			...
			for (var k = 0; k < countrySegments.length; k++) {
				...
				const countryName = countrySegment["name"]
	...
	drawPointLabels(latitude, longitude, searchLocationElevation, searchLocationCountry, searchLocationFarXY, labelLineLength, iso2FlagAspectRatios, bounds, "search")
	drawPointLabels(antipodeLatitude, antipodeLongitude, antipodeElevation, antipodeCountry, antipodeFarXY, labelLineLength, iso2FlagAspectRatios, bounds, "antipode")
	...
	if (numRows === 2) {
		...
		plotFlagRow(uniqueCountries.slice(Math.ceil(uniqueCountries.length / 2), uniqueCountries.length-1), 1, bounds, iso2FlagAspectRatios)
	} else {
	...
	function onMouseMove(e) {
		...
		if (y < (0.85 * DIMENSIONS.boundedRadius)) {
			...
			if (!cloakApplied || (cloakApplied && cloakCountry === mouseISO2.toLowerCase())) {
				...
				const mouseLongitude = (mouseIndex >= 10800) ? negativeLongitude : positiveLongitude
				...
				if (mouseIndex <= 10800) {
					const indexToLatitudeScale = d3.scaleLinear()
						.domain([0, 10800])
						.range([90, -90])
					mouseLatitude = indexToLatitudeScale(mouseIndex).toFixed(4)
				} else {
					const indexToLatitudeInverseScale = d3.scaleLinear()
						.domain([0, 10800])
						.range([-90, 90])
					mouseLatitude = indexToLatitudeInverseScale(mouseIndex - 10800).toFixed(4)
				}
				...
				drawPointLabels(mouseLatitude, mouseLongitude, mouseElevation, mouseCountry, mouseCoordsFar, labelLineLength, iso2FlagAspectRatios, bounds, "mouse", mouseLabelsDrawn)
				mouseLabelsDrawn = true

				// Update center information
				
				if (previousMouseISO2 !== mouseISO2) {
	...
	function onFlagClick(clickedFlag) {
		...
		cloakApplied = true
		d3.selectAll(`*[id^=cloak-${clickedISO2}`).style("opacity", 1)
	}
...
function drawPointLabels(latitude, longitude, elevation, country, farXY, labelLineLength, iso2FlagAspectRatios, bounds, prefix, update) {
		...
		if (update) {
			d3.select(`#${prefix}-label-name`)
				.attr("x", (longitude < 0) ? farXY[0] - labelLineLength - labelBuffer : farXY[0] + labelLineLength + labelBuffer)
				.attr("y", farXY[1] - 17)
				.attr("text-anchor", (longitude < 0) ?  "end" : "start")
				.html(`${formattedCountryName}`)
			d3.select(`#${prefix}-label-coords`)
				.attr("x", (longitude < 0) ? farXY[0] - labelLineLength - labelBuffer : farXY[0] + labelLineLength + labelBuffer)
				.attr("y", farXY[1])
				.attr("text-anchor", (longitude < 0) ?  "end" : "start")
				.html(`${Math.abs(latitude)}&deg;${latitudeDirection}, ${Math.abs(longitude)}&deg;${longitudeDirection}`)
			d3.select(`#${prefix}-label-elevation`)
				.attr("x", (longitude < 0) ? farXY[0] - labelLineLength - labelBuffer : farXY[0] + labelLineLength + labelBuffer)
				.attr("y", farXY[1] + 17)
				.attr("text-anchor", (longitude < 0) ?  "end" : "start")
				.html(`${numberWithCommas(Math.round(elevation * METERS_TO_FEET))} ft.`)
			d3.select(`#${prefix}-label-flag`)
				.attr("xlink:href", (formattedISO2 in iso2FlagAspectRatios) ? `./../assets/images/flags/${formattedISO2}.png` : '')
				.attr("x", (longitude < 0) ? farXY[0] - labelLineLength - flagWidth - labelBuffer: farXY[0] + labelLineLength + labelBuffer)
				.attr("y", farXY[1] + 25)
...
async function plotFlagRow(countries, rowIndex, bounds, iso2FlagAspectRatios) {
	...
	const countryFlagList = countryFlagGroup.selectAll(`*[id^=list-flag-${rowIndex}-`)
		.data(countries)
		.enter()
		.append("svg:image")
			.attr("id", (d,i) => `list-flag-${rowIndex}-${i}`)
			.attr("class", "list-flag")
			.attr("xlink:href", d => `./../assets/images/flags/${d.toLowerCase()}.png`)
			.attr("height", flagHeight)
			.style("justify-content", "center")
			.attr("x", (d,i) => {
	...
	const countryFlagBorder = countryFlagGroup.selectAll(`*[id^=border-flag-${rowIndex}-`)
		.data(countries)
		.enter()
		.append("rect")
			.attr("id", (d,i) => `border-flag-${rowIndex}-${i}`)
			.attr("class", "border-flag")
			.attr("height", flagHeight)
			.attr("width", (d,i) =>  {
				// Nepal has the only non-rectangular flag
	...
	d3.selectAll(`*[id^=list-flag-${rowIndex}-]`)
		.style("transform", `translate(${-flagCenteringAdjustment / 2}px, ${0}px)`)
		.style("opacity", 1)
	d3.selectAll(`*[id^=border-flag-${rowIndex}-]`)
		.style("transform", `translate(${-flagCenteringAdjustment / 2}px, ${0}px)`)
		.style("opacity", 1)
	return
}
